  ...
  const getResault = (_sequence=[]) => {
    ...
    if (Number(sequence[sequence.length - 1]) && sequence[0] !== undefined && !Object.keys(operators).includes(sequence[0])) {
      ...
      for (let i = 1; i < endPoint; i++) {
        newItem = operators[ResualtSequence[1]]
          ? operators[ResualtSequence[1]](
              Number(ResualtSequence[0]),
              Number(ResualtSequence[2])
            )
          : "";
        if (newItem) ResualtSequence = [newItem, ...ResualtSequence.slice(3)];
      }
      // set new history
    ...
    } else alert("Invalid Format Used");
  };
  ...
  return (
    <div className="container controllerContainer">
      <div className="row controllerRow">
        ...
        <div
          className="historyCol hide-on-small-only col s12 l4 z-depth-1 lighten-1 "
          id="historyCol"
        >
          <History
        ...
        <div
          className=" hide-on-med-and-up col s12 l4 z-depth-1 lighten-1 historyColMobile "
          id="historyColMobile"
        >
          <History
            newHistory={newHistory}
